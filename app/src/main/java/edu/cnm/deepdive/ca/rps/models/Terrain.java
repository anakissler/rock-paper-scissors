...
public class Terrain {
  ...
  public void combat() {
    ...
    int outcome = Breed.comparator.compare(attacker, defender); // if 1 is returned attacker wins, -1 attacker loses
    if (outcome > 0) {
  ...
  private int[] randomNeighbor(int row, int column) {
    ...
    return new int[] {
    (row + size + offset[0]) % size, // add offsets to them and % size makes sure it wraps around Toroidal space
